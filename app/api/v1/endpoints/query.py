...
from app.api.dependencies import DbSession, VectorStoreDep, LLMProviderDep
    ...
    if doc_ids:
        ...
        for d in doc_ids:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail={"message": f"Invalid doc_id: {d}", "error_code": "INVALID_DOCUMENT_IDS"},
                )
...
async def _format_sse(stream: AsyncGenerator[str, None]) -> AsyncGenerator[str, None]:
    ...
    try:
        async for chunk in stream:
            if chunk.startswith("\n\nSOURCES_JSON:\n"):
                # Strip marker precisely using slice, not replace
                marker = "\n\nSOURCES_JSON:\n"
                sources_json = chunk[len(marker):]
                # Compact JSON to ensure no embedded newlines in SSE data line
                sources_json = json.dumps(json.loads(sources_json), separators=(",", ":"))
                yield f"event: done\ndata: {sources_json}\n\n"
            else:
...
@router.post(
    ...
    description="Queries ingested documents using semantic search and LLM-powered answers.",
)
