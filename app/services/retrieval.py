...
from app.schemas.query import SourceReference, QueryResponse
from app.integrations.vectorstores.chroma import VectorStore, SearchResult
...
async def _resolve_filenames(db: AsyncSession, doc_ids: set[str]) -> dict[str, str]:
    ...
    for d in doc_ids:
        ...
        try:
            ...
            logger.warning("Skipping invalid UUID from vector store result: %s", d)
    if not doc_uuids:
        return {}
...
async def query_documents(
    ...
    # 1. Embed question
    query_embedding = await llm_provider.embed_text(question, model=llm_settings.default_embedding_model)
    ...
    # 6. Generate response
    if not stream:
        answer = await llm_provider.completion(prompt, SYSTEM_PROMPT, model=llm_settings.default_llm_model)
        return QueryResponse(answer=answer, sources=sources, confidence=confidence)
        
    async def stream_with_sources():
        async for chunk in llm_provider.stream_completion(prompt, SYSTEM_PROMPT, model=llm_settings.default_llm_model):
            yield chunk
